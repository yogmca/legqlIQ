...
server {
    ...
    # IMPORTANT: proxy_pass should be http://127.0.0.1:4000/ (with trailing slash, no /api/)
...
sudo cp /tmp/legaliq-final.conf /etc/nginx/sites-available/legaliq
...
if [ $? -eq 0 ]; then
    ...
    echo "✓ Done! The 413 error should be fixed now."
    echo ""
